...
def train_RFL_DFDC(data_obj, act_prob,rho,
    ...
    suffix += '_a%f_b%f_rho%f_S%d_F%f_Lr%f_%d_%f_B%d_E%d_W%f_g%f' % (alpha_coef,beta,rho,
    ...
    writer = SummaryWriter('%sRuns_RFL_DFDC/%s/%s' % (data_path, data_obj.name, suffix[:]))
    ...
    not os.path.exists('%sModel/%s/%s/cld_avg_%dcom.pt' % (data_path, data_obj.name, suffix, com_amount))):
        ...
        for i in range(saved_itr + 1, com_amount):
            ...
            for clnt in selected_clnts:
                ...
                clnt_models[clnt] = train_model_RFL_DFDC(model, model_func, alpha_coef_adpt,rho,
...
def train_RFL_DFDC_wo_hot(data_obj, act_prob,rho,
                 learning_rate, batch_size, epoch, com_amount, print_per,
                 weight_decay, model_func, init_model, alpha_coef, beta,
                 sch_step, sch_gamma, save_period,
                 suffix='', trial=True, data_path='', rand_seed=0, lr_decay_per_round=1,
                 sim_gamma=0.1):
    suffix = 'RFL_DFDC_wo_hot_' + suffix
    suffix += '_a%f_b%f_rho%f_S%d_F%f_Lr%f_%d_%f_B%d_E%d_W%f_g%f' % (alpha_coef,beta,rho,
        save_period, act_prob, learning_rate, sch_step, sch_gamma, batch_size,
        epoch, weight_decay, sim_gamma)
    suffix += '_seed%d' % rand_seed
...
def train_RFL_DFDC_wo_ta(data_obj, act_prob,rho,
                 learning_rate, batch_size, epoch, com_amount, print_per,
                 weight_decay, model_func, init_model, alpha_coef, beta,
                 sch_step, sch_gamma, save_period,
                 suffix='', trial=True, data_path='', rand_seed=0, lr_decay_per_round=1,
                 sim_gamma=0.1):
    suffix = 'RFL_DFDC_wo_ta_' + suffix
    suffix += '_a%f_b%f_rho%f_S%d_F%f_Lr%f_%d_%f_B%d_E%d_W%f_g%f' % (alpha_coef,beta,rho,
        save_period, act_prob, learning_rate, sch_step, sch_gamma, batch_size,
        epoch, weight_decay, sim_gamma)
    suffix += '_seed%d' % rand_seed
...
def train_RFL_DFDC_mean_fill(data_obj, act_prob,rho,
                 learning_rate, batch_size, epoch, com_amount, print_per,
                 weight_decay, model_func, init_model, alpha_coef, beta,
                 sch_step, sch_gamma, save_period,
                 suffix='', trial=True, data_path='', rand_seed=0, lr_decay_per_round=1,
                 sim_gamma=0.1):
    suffix = 'RFL_DFDC_mean_fill' + suffix
    suffix += '_a%f_b%f_rho%f_S%d_F%f_Lr%f_%d_%f_B%d_E%d_W%f_g%f' % (alpha_coef,beta,rho,
        save_period, act_prob, learning_rate, sch_step, sch_gamma, batch_size,
        epoch, weight_decay, sim_gamma)
    suffix += '_seed%d' % rand_seed
...
def train_RFL_DFDC_ua(data_obj, act_prob,rho,
                 learning_rate, batch_size, epoch, com_amount, print_per,
                 weight_decay, model_func, init_model, alpha_coef, beta,
                 sch_step, sch_gamma, save_period,
                 suffix='', trial=True, data_path='', rand_seed=0, lr_decay_per_round=1,
                 sim_gamma=0.1):
    suffix = 'RFL_DFDC_ua' + suffix
    suffix += '_a%f_b%f_rho%f_S%d_F%f_Lr%f_%d_%f_B%d_E%d_W%f_g%f' % (alpha_coef,beta,rho,
        save_period, act_prob, learning_rate, sch_step, sch_gamma, batch_size,
        epoch, weight_decay, sim_gamma)
    suffix += '_seed%d' % rand_seed
    ...
    not os.path.exists('%sModel/%s/%s/cld_avg_%dcom.pt' % (data_path, data_obj.name, suffix, com_amount))):
        ...
        for i in range(saved_itr + 1, com_amount):
            ...
            for clnt in unselected_clnts:
                neighbor_weights = np.ones(len(selected_clnts),dtype=float) if len(selected_clnts) > 0 else np.array([])
                weight_sum = np.sum(neighbor_weights)
                if len(neighbor_weights) == 0 or weight_sum <= 1e-12:
                    continue
                normalized = neighbor_weights / weight_sum
...
def train_model_RFL_DFDC(model, model_func, alpha, rho,global_model_param, parameter_drifts_curr,
                            trn_x, trn_y,
