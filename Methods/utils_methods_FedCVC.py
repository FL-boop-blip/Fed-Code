...
def train_FedCVC_SGD(data_obj, act_prob,
    ...
    suffix = 'FedCVC_SGD_' + suffix
    suffix += '_a%f_b%f_S%d_F%f_Lr%f_%d_%f_B%d_E%d_W%f_a%f' % (
    alpha_coef,beta, save_period, act_prob, learning_rate, sch_step, sch_gamma, batch_size, epoch, weight_decay)
    ...
    writer = SummaryWriter('%sRuns_FedCVC_SGD/%s/%s' % (data_path, data_obj.name, suffix[:]))
    ...
    not os.path.exists('%sModel/%s/%s/cld_avg_%dcom.pt' % (data_path, data_obj.name, suffix, com_amount))):
        ...
        for i in range(saved_itr + 1, com_amount):
            ...
            for clnt in selected_clnts:
                ...
                clnt_models[clnt] = train_model_FedCVC_SGD(model, model_func, alpha_coef_adpt,
...
def train_FedCVC(data_obj, act_prob,
                 learning_rate, batch_size, epoch, beta,com_amount, print_per,
                 weight_decay, model_func, init_model, alpha_coef,rho,
                 sch_step, sch_gamma, save_period,
                 suffix='', trial=True, data_path='', rand_seed=0, lr_decay_per_round=1):
    suffix = 'FedCVC_' + suffix
    suffix += '_a%f_b%f_S%d_F%f_Lr%f_%d_%f_B%d_E%d_W%f_a%f' % (
    alpha_coef, beta, save_period, act_prob, learning_rate, sch_step, sch_gamma, batch_size, epoch, weight_decay)
    suffix += '_seed%d' % rand_seed
    suffix += '_lrdecay%f' % lr_decay_per_round
    ...
    # writer object is for tensorboard visualization, comment out if not needed
    writer = SummaryWriter('%sRuns_FedCVC/%s/%s' % (data_path, data_obj.name, suffix[:]))
    log_dir = os.path.join(writer.logdir, 'Logs') if hasattr(writer, 'logdir') and writer.logdir else 'Logs'
...
def train_model_FedCVC_SGD(model, model_func, alpha, global_model_param, hist_i,
                      trn_x, trn_y,
                      learning_rate, batch_size, epoch, print_per,
                      weight_decay, dataset_name, sch_step, sch_gamma):
    n_trn = trn_x.shape[0]
