...
def train_SCAFFOLD(data_obj, act_prob ,learning_rate, batch_size, n_minibatch,
    ...
    writer = SummaryWriter('%sRuns_SCAFFOLD/%s/%s' %(data_path, data_obj.name, suffix[:]))
    ...
    if (trial) or (not os.path.exists('%sModel/%s/%s/%dcom_sel.pt' %(data_path, data_obj.name, suffix, com_amount))):
        ...
        for i in range(saved_itr+1, com_amount):
            ...
            prev_global_dual = update_residual_metrics(residual_metrics, i, clnt_params_list, selected_clnts,
                                                       avg_model_params, prev_params, prev_global_dual)
            log_residual_scalars(writer, 'Sel clients', residual_metrics[i], i)
            prev_global_param = avg_model_params.copy()
